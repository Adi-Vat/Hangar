...
public class HandAnimator : MonoBehaviour
{
    ...
    void CacheBones()
    {
        ...
        {
            // Set the "relaxed rotaitons" to the starting rotations
    ...
    void ReadTriggerInput()
    {
        ...
        for (int i = 0; i < chainLength; i++)
        {
            indexFingerAngles[i] = Quaternion.Slerp(relaxedIndexFingerRotations[i], Quaternion.Euler(contractedIndexFingerRotations[i]), triggerValue);
        }
    }
    ...
    void ReadGripInput()
    {
        ...
        for (int i = 0; i < chainLength; i++)
        {
            otherFingerAngles[i] = Quaternion.Slerp(relaxedOtherFingerRotations[i], Quaternion.Euler(contractedOtherFingerRotations[i]), gripValue);
        }
    }
    ...
    void ReadThumbstickInput()
    {
        ...
        for (int i = 0; i < chainLength; i++)
        {
            thumbAngles[i] = Quaternion.Slerp(relaxedThumbRotations[i], Quaternion.Euler(contractedThumbRotations[i]), thumbstickHoverValue);
        }
    }
